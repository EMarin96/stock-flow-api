...
/// <summary>
/// Builds a single <see cref="ProductApiFactory"/> (and its TestServer/host)
...
/// [Collection("Postgres collection")] and IClassFixture&lt;ProductApiFactoryFixture&gt;.
/// Migrations run once here, in <see cref="InitializeAsync"/>; per-test
/// cleanup (TRUNCATE) stays the test class's responsibility.
/// </summary>
public sealed class ProductApiFactoryFixture : IAsyncLifetime
{
    public ProductApiFactory Factory { get; }

    public ProductApiFactoryFixture(PostgresContainerFixture postgresFixture)
    {
        Factory = new ProductApiFactory(postgresFixture.ConnectionString);
    }
