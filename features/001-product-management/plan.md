...
- **Repository pattern (write-side) + Unit of Work, adopted after initial implementation** — the first pass had `ProductWriteRepository.SaveChangesAsync()` translating the Postgres unique-constraint violation into `DbUpdateUniqueConstraintException`. That translation would have had to be duplicated in every future write repository (`LocationWriteRepository`, `StockMovementWriteRepository`, …). Introducing `IUnitOfWork` centralizes the commit and the DB-error translation in one place, and sets up atomic multi-repository commits, which `StockMovement TRANSFER` will need (it touches two `Location`-scoped balances at once). Discarded alternative: an EF Core `SaveChangesInterceptor`, which would have solved only the duplication without the multi-entity atomicity benefit — rejected in favor of the option that also serves the near-term `TRANSFER` need.
- **`Sku` and `Money` as Value Objects, adopted after initial implementation** — `Product.Sku` (plain `string`) and `Product.Price` (plain `decimal`) were converted into Domain Value Objects to make their invariants explicit and unrepresentable-by-construction. `Money` also gained real multi-currency support: `Amount` + `Currency` (a closed enum, `USD`/`CRC` for now). Both VOs self-validate via guard clauses that throw a single generic `DomainValidationException` (defined in `Domain`) rather than returning the Application-layer `Result<T>` — using `Result<T>` here would force `Domain` to reference `Application`, inverting Clean Architecture's dependency rule. These guard clauses are unreachable in normal flow (FluentValidation already validates at the Application boundary before `Product.Create`/`Update` run); they exist as defense-in-depth, not business control flow. Primitive→VO conversion happens only inside `Product` — `Command`/`Validator`/`Api` keep using `string`/`decimal` for SKU/price, since that's what arrives over JSON. `Currency` is the one exception: as a dependency-free enum, it flows as a typed field through `Command`/`Validator`/`Api` too, rather than being stringly-typed at those boundaries.
